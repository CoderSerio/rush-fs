...
const {
  codes: { ERR_DIR_CLOSED, ERR_DIR_CONCURRENT_OPERATION, ERR_INVALID_THIS, ERR_MISSING_ARGS },
} = require('internal/errors')

const { FSReqCallback } = binding
const { promisify } = require('internal/util')
const { getDirent, getOptions, getValidatedPath } = require('internal/fs/utils')
...
class Dir {
  ...
  constructor(handle, path, options) {
    ...
    this.#readPromisified = FunctionPrototypeBind(promisify(this.#readImpl), this, false)
    this.#closePromisified = FunctionPrototypeBind(promisify(this.close), this)
  }

  get path() {
    if (!(#path in this)) throw new ERR_INVALID_THIS('Dir')
    return this.#path
  }
  ...
  #processReadResult(path, result) {
    for (let i = 0; i < result.length; i += 2) {
      ArrayPrototypePush(this.#bufferedEntries, getDirent(path, result[i], result[i + 1]))
    }
  }

  #readSyncRecursive(dirent) {
    const path = pathModule.join(dirent.parentPath, dirent.name)
    const handle = dirBinding.opendir(path, this.#options.encoding)
