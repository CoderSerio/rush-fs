...
const {
  constants: { kIoMaxLength, kMaxUserId, kReadFileBufferLength, kReadFileUnknownBufferLength, kWriteFileMaxChunkSize },
  copyObject,
  getDirents,
  getOptions,
...
const { getLazy, kEmptyObject, lazyDOMException, promisify, isWindows, isMacOS } = require('internal/util')
...
const lazyReadableStream = getLazy(() => require('internal/webstreams/readablestream').ReadableStream)
...
async function copyFile(src, dest, mode) {
  return await PromisePrototypeThen(
    binding.copyFile(getValidatedPath(src, 'src'), getValidatedPath(dest, 'dest'), mode, kUsePromises),
    undefined,
    handleErrorFromBinding,
  )
}
...
async function write(handle, buffer, offsetOrOptions, length, position) {
  ...
  if (isArrayBufferView(buffer)) {
    ...
    const bytesWritten =
      (await PromisePrototypeThen(
        binding.writeBuffer(handle.fd, buffer, offset, length, position, kUsePromises),
        undefined,
        handleErrorFromBinding,
      )) || 0
...
async function rename(oldPath, newPath) {
  ...
  return await PromisePrototypeThen(binding.rename(oldPath, newPath, kUsePromises), undefined, handleErrorFromBinding)
}
...
async function ftruncate(handle, len = 0) {
  ...
  len = MathMax(0, len)
  return await PromisePrototypeThen(binding.ftruncate(handle.fd, len, kUsePromises), undefined, handleErrorFromBinding)
}
...
async function rmdir(path, options) {
  ...
  return await PromisePrototypeThen(binding.rmdir(path, kUsePromises), undefined, handleErrorFromBinding)
}

async function fdatasync(handle) {
  return await PromisePrototypeThen(binding.fdatasync(handle.fd, kUsePromises), undefined, handleErrorFromBinding)
}

async function fsync(handle) {
  return await PromisePrototypeThen(binding.fsync(handle.fd, kUsePromises), undefined, handleErrorFromBinding)
}
...
async function mkdir(path, options) {
  ...
  return await PromisePrototypeThen(
    binding.mkdir(path, parseFileMode(mode, 'mode', 0o777), recursive, kUsePromises),
    undefined,
    handleErrorFromBinding,
  )
}
...
async function readdirRecursive(originalPath, options) {
  ...
  const queue = [
    [
      originalPath,
      await PromisePrototypeThen(
        binding.readdir(originalPath, options.encoding, !!options.withFileTypes, kUsePromises),
        undefined,
        handleErrorFromBinding,
      ),
    ],
      ...
      for (const ent of readdir) {
        ...
        if (stat === 1) {
          ArrayPrototypePush(queue, [
            direntPath,
            await PromisePrototypeThen(
              binding.readdir(direntPath, options.encoding, false, kUsePromises),
              undefined,
              handleErrorFromBinding,
            ),
          ])
...
async function readdir(path, options) {
  ...
  const result = await PromisePrototypeThen(
    binding.readdir(path, options.encoding, !!options.withFileTypes, kUsePromises),
    undefined,
    handleErrorFromBinding,
  )
...
async function symlink(target, path, type) {
  ...
  path = getValidatedPath(path)
  return await PromisePrototypeThen(
    binding.symlink(preprocessSymlinkDestination(target, type, path), path, stringToSymlinkType(type), kUsePromises),
    undefined,
    handleErrorFromBinding,
  )
}
...
async function fchmod(handle, mode) {
  ...
  return await PromisePrototypeThen(binding.fchmod(handle.fd, mode, kUsePromises), undefined, handleErrorFromBinding)
}

async function chmod(path, mode) {
  path = getValidatedPath(path)
  mode = parseFileMode(mode, 'mode')
  return await PromisePrototypeThen(binding.chmod(path, mode, kUsePromises), undefined, handleErrorFromBinding)
}
...
async function lchown(path, uid, gid) {
  ...
  return await PromisePrototypeThen(binding.lchown(path, uid, gid, kUsePromises), undefined, handleErrorFromBinding)
}
...
async function chown(path, uid, gid) {
  ...
  validateInteger(uid, 'uid', -1, kMaxUserId)
  validateInteger(gid, 'gid', -1, kMaxUserId)
  return await PromisePrototypeThen(binding.chown(path, uid, gid, kUsePromises), undefined, handleErrorFromBinding)
}

async function utimes(path, atime, mtime) {
  path = getValidatedPath(path)
  return await PromisePrototypeThen(
    binding.utimes(path, toUnixTimestamp(atime), toUnixTimestamp(mtime), kUsePromises),
    undefined,
    handleErrorFromBinding,
  )
}
...
async function lutimes(path, atime, mtime) {
  return await PromisePrototypeThen(
    binding.lutimes(getValidatedPath(path), toUnixTimestamp(atime), toUnixTimestamp(mtime), kUsePromises),
    undefined,
    handleErrorFromBinding,
  )
}
