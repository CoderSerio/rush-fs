...
const {
  UVException,
  codes: { ERR_FS_EISDIR, ERR_INCOMPATIBLE_OPTION_PAIR, ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE, ERR_OUT_OF_RANGE },
  hideStackFrames,
} = require('internal/errors')
const { isArrayBufferView, isBigInt64Array, isDate, isUint8Array } = require('internal/util/types')
const { kEmptyObject, once, deprecate, isWindows } = require('internal/util')
...
const validateRmOptionsSync = hideStackFrames((path, options, expectDir) => {
  ...
  if (!options.force || expectDir || !options.recursive) {
    const isDirectory = lazyLoadFs().lstatSync(path, { throwIfNoEntry: !options.force })?.isDirectory()
...
const validateRmdirOptions = hideStackFrames((options, defaults = { __proto__: null }) => {
