...
Single-file operations have a ~0.3 µs napi bridge overhead. **Recursive glob** on a small/medium tree (e.g. `**/*.rs` over the repo) can be on par with node-glob; on **large, deep trees** Rush-FS wins (see row above). Other operations:
...
| `glob` recursive (`**/*.rs`, small tree) vs node-glob | ~22 ms | ~40 ms | ~1.8x (node-glob faster on this scale) |
