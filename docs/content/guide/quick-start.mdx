...
Import the APIs you need from `rush-fs` — same names as `node:fs`:
...
- **Sync** — Supported APIs expose a sync variant with the `Sync` suffix (e.g. `readdirSync`, `statSync`). See [API Overview](/docs/api) for the full list.
